...
const queryPredict1YBQXRMS = ref("");
const fetchqueryPredict1YBQXRMS = async () => {
  const res = await getqueryPredict1YBQXRMS();
  // 检查数组是否非空
  if (res.data.length > 0) {
    ...
    queryPredict1YBQXRMS.value = "0.00"; // 如果数组为空，可以设置为 "0.00" 或其他默认值
  }
  console.log(res, "res"); 
};
fetchqueryPredict1YBQXRMS(); 
