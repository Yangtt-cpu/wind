    ...
    <div class="relative h-full w-full rounded-lg bg-blue-50">
      ...
      >
        ...
        <div>
          ...
          ><span style="font-weight: bold">{{ queryPredict4SBQXRMS }}mm/s</span>
        </div>
        <div>
          <span style="margin-right: 40px">Y轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4SBQYRMS }}mm/s</span>
        </div>
      </div>
      <div
        style="
      ...
      >
        ...
        <div>
          <span style="margin-right: 40px">X轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4SBFXRMS }}mm/s</span>
        </div>
        <div>
          <span style="margin-right: 40px">Y轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4SBFYRMS }}mm/s</span>
        </div>
      </div>
...
import {
  getqueryPredict4SBQXRMS,
  getqueryPredict4SBQYRMS,
  getqueryPredict4SBFXRMS,
  getqueryPredict4SBFYRMS,
  getqueryAlert
} from "@/api/routes";
import { ref, onMounted, onBeforeUnmount } from "vue";

// 定义响应式数据
const queryPredict4SBQXRMS = ref("0.00");
const queryPredict4SBQYRMS = ref("0.00");
const queryPredict4SBFXRMS = ref("0.00");
const queryPredict4SBFYRMS = ref("0.00");
...
const fetchData = async () => {
  try {
    ...
    const [res1, res2, res3, res4, alertRes] = await Promise.all([
      ...
      getqueryPredict4SBFXRMS(),
      getqueryPredict4SBFYRMS(),
      getqueryAlert("4SBalert")
    ]);

    // 更新 4SAQXRMS 数据
    if (res1.data?.length > 0) {
      const lastItem = res1.data[res1.data.length - 1];
      queryPredict4SBQXRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4SBQXRMS.value = "0.00";
    }
    // 更新 4SAQYRMS 数据
    if (res2.data?.length > 0) {
      const lastItem = res2.data[res2.data.length - 1];
      queryPredict4SBQYRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4SBQYRMS.value = "0.00";
    }
    // 更新 4SAFXRMS 数据
    if (res3.data?.length > 0) {
      const lastItem = res3.data[res3.data.length - 1];
      queryPredict4SBFXRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4SBFXRMS.value = "0.00";
    }
    // 更新 4SAFYRMS 数据
    if (res4.data?.length > 0) {
      const lastItem = res4.data[res4.data.length - 1];
      queryPredictASBFYRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4SBFYRMS.value = "0.00";
    }
