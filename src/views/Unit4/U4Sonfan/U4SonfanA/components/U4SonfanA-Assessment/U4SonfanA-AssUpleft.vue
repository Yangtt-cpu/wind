    ...
    <div class="relative h-full w-full rounded-lg bg-blue-50">
      ...
      >
        ...
        <div>
          ...
          ><span style="font-weight: bold">{{ queryPredict4SAQXRMS }}mm/s</span>
        </div>
        <div>
          <span style="margin-right: 40px">Y轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4SAQYRMS }}mm/s</span>
        </div>
      </div>
      <div
        style="
      ...
      >
        ...
        <div>
          <span style="margin-right: 40px">X轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4SAFXRMS }}mm/s</span>
        </div>
        <div>
          <span style="margin-right: 40px">Y轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4SAFYRMS }}mm/s</span>
        </div>
      </div>
...
import {
  getqueryPredict4SAQXRMS,
  getqueryPredict4SAQYRMS,
  getqueryPredict4SAFXRMS,
  getqueryPredict4SAFYRMS,
  getqueryAlert
} from "@/api/routes";
import { ref, onMounted, onBeforeUnmount } from "vue";

// 定义响应式数据
const queryPredict4SAQXRMS = ref("0.00");
const queryPredict4SAQYRMS = ref("0.00");
const queryPredict4SAFXRMS = ref("0.00");
const queryPredict4SAFYRMS = ref("0.00");
...
const fetchData = async () => {
  try {
    ...
    const [res1, res2, res3, res4, alertRes] = await Promise.all([
      ...
      getqueryPredict4SAFXRMS(),
      getqueryPredict4SAFYRMS(),
      getqueryAlert("4SAalert")
    ]);

    // 更新 4SAQXRMS 数据
    if (res1.data?.length > 0) {
      const lastItem = res1.data[res1.data.length - 1];
      queryPredict4SAQXRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4SAQXRMS.value = "0.00";
    }
    // 更新 4SAQYRMS 数据
    if (res2.data?.length > 0) {
      const lastItem = res2.data[res2.data.length - 1];
      queryPredict4SAQYRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4SAQYRMS.value = "0.00";
    }
    // 更新 4SAFXRMS 数据
    if (res3.data?.length > 0) {
      const lastItem = res3.data[res3.data.length - 1];
      queryPredict4SAFXRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4SAFXRMS.value = "0.00";
    }
    // 更新 4SAFYRMS 数据
    if (res4.data?.length > 0) {
      const lastItem = res4.data[res4.data.length - 1];
      queryPredictASAFYRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4SAFYRMS.value = "0.00";
    }
