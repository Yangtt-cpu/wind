<template>
  ...
  >
    ...
    <div class="relative h-full w-full rounded-lg bg-blue-50">
      <!-- 主图像 -->
      <img
        name="U4PrimaryfanBAssUpleft-img"
      ...
      >
        ...
        <div>
          ...
          ><span style="font-weight: bold">{{ queryPredict4YCBQXRMS }}mm/s</span>
        </div>
        <div>
          <span style="margin-right: 40px">Y轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4YCBQYRMS }}mm/s</span>
        </div>
      </div>
      <div
        style="
      ...
      >
        ...
        <div>
          <span style="margin-right: 40px">X轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4YCBFXRMS }}mm/s</span>
        </div>
        <div>
          <span style="margin-right: 40px">Y轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4YCBFYRMS }}mm/s</span>
        </div>
      </div>
...
import {
  getqueryPredict4YCBQXRMS,
  getqueryPredict4YCBQYRMS,
  getqueryPredict4YCBFXRMS,
  getqueryPredict4YCBFYRMS,
  getqueryAlert
} from "@/api/routes";
import { ref, onMounted, onBeforeUnmount } from "vue";

// 定义响应式数据
const queryPredict4YCBQXRMS = ref("0.00");
const queryPredict4YCBQYRMS = ref("0.00");
const queryPredict4YCBFXRMS = ref("0.00");
const queryPredict4YCBFYRMS = ref("0.00");
...
const fetchData = async () => {
  try {
    ...
    const [res1, res2, res3, res4, alertRes] = await Promise.all([
      ...
      getqueryPredict4YCBFXRMS(),
      getqueryPredict4YCBFYRMS(),
      getqueryAlert("4YCBalert")
    ]);

    // 更新 4YAQXRMS 数据
    if (res1.data?.length > 0) {
      const lastItem = res1.data[res1.data.length - 1];
      queryPredict4YCBQXRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YCBQXRMS.value = "0.00";
    }
    // 更新 4YAQYRMS 数据
    if (res2.data?.length > 0) {
      const lastItem = res2.data[res2.data.length - 1];
      queryPredict4YCBQYRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YCBQYRMS.value = "0.00";
    }
    // 更新 4YAFXRMS 数据
    if (res3.data?.length > 0) {
      const lastItem = res3.data[res3.data.length - 1];
      queryPredict4YCBFXRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YCBFXRMS.value = "0.00";
    }
    // 更新 4YAFYRMS 数据
    if (res4.data?.length > 0) {
      const lastItem = res4.data[res4.data.length - 1];
      queryPredictAYCBFYRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YCBFYRMS.value = "0.00";
    }

    if (alertRes?.data?.length > 0) {
      const alert = alertRes.data[0]; // 获取第一个数据对象
