    ...
    <div class="relative h-full w-full rounded-lg bg-blue-50">
      ...
      >
        ...
        <div>
          ...
          ><span style="font-weight: bold">{{ queryPredict4YAQXRMS }}mm/s</span>
        </div>
        <div>
          <span style="margin-right: 40px">Y轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4YAQYRMS }}mm/s</span>
        </div>
      </div>
      <div
        style="
      ...
      >
        ...
        <div>
          <span style="margin-right: 40px">X轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4YAFXRMS }}mm/s</span>
        </div>
        <div>
          <span style="margin-right: 40px">Y轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4YAFYRMS }}mm/s</span>
        </div>
      </div>
...
import {
  getqueryPredict4YAQXRMS,
  getqueryPredict4YAQYRMS,
  getqueryPredict4YAFXRMS,
  getqueryPredict4YAFYRMS,
  getqueryAlert
} from "@/api/routes";
import { ref, onMounted, onBeforeUnmount } from "vue";

// 定义响应式数据
const queryPredict4YAQXRMS = ref("0.00");
const queryPredict4YAQYRMS = ref("0.00");
const queryPredict4YAFXRMS = ref("0.00");
const queryPredict4YAFYRMS = ref("0.00");
...
const fetchData = async () => {
  try {
    ...
    const [res1, res2, res3, res4, alertRes] = await Promise.all([
      ...
      getqueryPredict4YAFXRMS(),
      getqueryPredict4YAFYRMS(),
      getqueryAlert("4YAalert")
    ]);

    // 更新 4YAQXRMS 数据
    if (res1.data?.length > 0) {
      const lastItem = res1.data[res1.data.length - 1];
      queryPredict4YAQXRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YAQXRMS.value = "0.00";
    }
    // 更新 4YAQYRMS 数据
    if (res2.data?.length > 0) {
      const lastItem = res2.data[res2.data.length - 1];
      queryPredictAYAQYRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YAQYRMS.value = "0.00";
    }
    // 更新 4YAFXRMS 数据
    if (res1.data?.length > 0) {
      const lastItem = res1.data[res1.data.length - 1];
      queryPredict4YAFXRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YAFXRMS.value = "0.00";
    }
    // 更新 4YAFYRMS 数据
    if (res2.data?.length > 0) {
      const lastItem = res2.data[res2.data.length - 1];
      queryPredictAYAFYRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YAFYRMS.value = "0.00";
    }
