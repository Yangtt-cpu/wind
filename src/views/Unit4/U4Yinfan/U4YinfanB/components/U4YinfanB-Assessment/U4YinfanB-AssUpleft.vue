    ...
    <div class="relative h-full w-full rounded-lg bg-blue-50">
      ...
      >
        ...
        <div>
          ...
          ><span style="font-weight: bold">{{ queryPredict4YBQXRMS }}mm/s</span>
        </div>
        <div>
          <span style="margin-right: 40px">Y轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4YBQYRMS }}mm/s</span>
        </div>
      </div>
      <div
        style="
      ...
      >
        ...
        <div>
          <span style="margin-right: 40px">X轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4YBFXRMS }}mm/s</span>
        </div>
        <div>
          <span style="margin-right: 40px">Y轴位移</span
          ><span style="font-weight: bold">{{ queryPredict4YBFYRMS }}mm/s</span>
        </div>
      </div>
...
import {
  getqueryPredict4YBQXRMS,
  getqueryPredict4YBQYRMS,
  getqueryPredict4YBFXRMS,
  getqueryPredict4YBFYRMS,
  getqueryAlert
} from "@/api/routes";
import { ref, onMounted, onBeforeUnmount } from "vue";

// 定义响应式数据
const queryPredict4YBQXRMS = ref("0.00");
const queryPredict4YBQYRMS = ref("0.00");
const queryPredict4YBFXRMS = ref("0.00");
const queryPredict4YBFYRMS = ref("0.00");
...
const fetchData = async () => {
  try {
    ...
    const [res1, res2, res3, res4, alertRes] = await Promise.all([
      ...
      getqueryPredict4YBFXRMS(),
      getqueryPredict4YBFYRMS(),
      getqueryAlert("4YBalert")
    ]);

    // 更新 4YAQXRMS 数据
    if (res1.data?.length > 0) {
      const lastItem = res1.data[res1.data.length - 1];
      queryPredict4YBQXRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YBQXRMS.value = "0.00";
    }
    // 更新 4YAQYRMS 数据
    if (res2.data?.length > 0) {
      const lastItem = res2.data[res2.data.length - 1];
      queryPredict4YBQYRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YBQYRMS.value = "0.00";
    }
    // 更新 4YAFXRMS 数据
    if (res3.data?.length > 0) {
      const lastItem = res3.data[res3.data.length - 1];
      queryPredict4YBFXRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YBFXRMS.value = "0.00";
    }
    // 更新 4YAFYRMS 数据
    if (res4.data?.length > 0) {
      const lastItem = res4.data[res4.data.length - 1];
      queryPredictAYBFYRMS.value = parseFloat(lastItem.value).toFixed(2);
    } else {
      queryPredict4YBFYRMS.value = "0.00";
    }
